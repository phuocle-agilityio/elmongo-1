...
var VERSIONED_URI = process.env.VERSIONED_URI || undefined;
...
module.exports = function (schema, options, cb) {
    ...
    // For sync data to old index
    if (VERSIONED_URI) {
      console.log('Exists VERSIONED_URI value');
      versionedIndexName = versionedUri;
    }
    ...
    async.series({
        ...
        populateVersionedIndex: function (next) {
            ...
            var commandSequence = [], synced = 0
              self.count(function (error, count) {
                ...
                // Ex: for run continue from last object id
                // QUERY = {
                //   '_id': {'$gt': '5f18033f76540901003712af'}
                // };

                console.log('QUERY: ', QUERY);

                // stream docs - and upload in batches of size BATCH_SIZE
                var docStream = self.find(QUERY).sort('_id').stream();
                ...
                docStream.on('data', function (doc) {
                    ...
                    if (commandSequence.length === BATCH_SIZE) {
                      ...
                      async.parallelLimit(jobs, JOB_NUM, function (err) {
                        ...
                        console.log('Number of records had sync: ', synced);
                        console.log('Last object Id: ', strObjectId);
        ...
        replaceAliases: function (next) {
            ...
            // For replace aliases a case sync to index exists
            // if (VERSIONED_URI) {
            //   return next();
            // }
